...
/** Pre-fills the iOS/Android mail composer with the PDF attached.
 *  Falls back to the system share sheet (Outlook, Gmail, etc.) if the
 *  native Mail app is not set up on the device. */
export async function emailReport(inspection: Inspection, pdfUri: string): Promise<void> {
  const available = await MailComposer.isAvailableAsync();
  ...
  if (available) {
    ...
    // Fall back to share sheet so user can send via Outlook, Gmail, etc.
    const canShare = await Sharing.isAvailableAsync();
    if (!canShare) {
      throw new Error('Neither Mail nor Sharing is available on this device.');
    }
    await Sharing.shareAsync(pdfUri, {
      mimeType: 'application/pdf',
